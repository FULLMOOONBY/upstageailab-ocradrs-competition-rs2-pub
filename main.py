...
from model.lightgbm.train_lgbm import run_lightgbm
...
def main(cfg: DictConfig) -> None:
    ...
    elif cfg.model.type.lower() == "lightgbm":  # LightGBM 모델 추가
        log.info("LightGBM 모델 평가 시작...")
        # model_func = run_lgbm
        model_specific_config = {
            "model_type": "lightgbm",
            "lightgbm": OmegaConf.to_container(cfg.model.lightgbm, resolve=True),
            "recommendation": OmegaConf.to_container(cfg.recommendation, resolve=True)
        }
        recommendations_df = run_lightgbm(cfg)  # 전체 설정 객체 전달
